...
// GET /api/search?q=키워드&maxResults=20&order=viewCount&regionCode=KR&publishedAfter=ISO_DATE
router.get("/search", async (req, res) => {
    try {
        const { q, maxResults, order, regionCode, publishedAfter } = req.query;
        ...
        const rows = videos.map((v) => {
            ...
            const ratio =
                subs && subs > 0 ? Number((viewCount / subs).toFixed(4)) : null;
